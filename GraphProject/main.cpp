...
int main(){
    ...
    srand(time(NULL));
    RenderWindow window;
    Vector2i centerWindow((VideoMode::getDesktopMode().width / 2)-900, (VideoMode::getDesktopMode().height /2)-650);
    window.create(VideoMode(1800,1100), "Lego Meta", Style::Titlebar | Style::Close);
    window.setPosition(centerWindow);
    window.setKeyRepeatEnabled(true);

    Textbox textbox1(25, Color::Black,false);
    Font arial;
    ...
    Textbox textbox2(25, Color::Black,false);
    textbox2.setFont(arial);
    textbox2.setPosition({250,350});
    textbox2.setLimit(true,30);

    Button text1("Search Name 1", {425,50}, 20, Color::Green, Color::Black);
    ...
    Text person1, person2, person3, person4, person5, person6, person7, person8, person9;
    ...
    profilePic5.setPosition({1200,350});
    ...
    randName.setString("No Name Generated Yet");
            ...
            switch (Event.type) {
                ...
                case Event::MouseMoved:
                    if(SearchButton.isMouseOver(window)){
                        ...
                        SearchButton.setBackColor(Color::Green);
                    }
                ...
                case Event::MouseButtonPressed:
                    ...
                    if(SearchButton.isMouseOver(window)){
                        if(textbox1.numCharTyped!=0 && textbox2.numCharTyped!=0){
                            ...
                            if(OnOffButton && currentName1==textbox1.nameTyped && currentName2==textbox2.nameTyped){
                                ...
                                searchText.setString("You Have Already Searched These Names With BFS!");
                            ...
                            vector<string> names;
                            names.resize(9);
                            unordered_map<string,int>::const_iterator got = randGraph.stringmap.find(name1);
                            ...
                            if(got==randGraph.stringmap.end() || got2==randGraph.stringmap.end()){
                                ...
                                happy.setPosition({412.5,600});
                                searchText.setFillColor(Color::Red);
                                searchText.setString("Entered Names Not Found!");
                                OnOffButton=true;
                                break;
                            }
                            start = std::chrono::system_clock::now();
                            connectionFound = randGraph.bfsBool(got->second,got2->second);
                            ...
                            if(connectionFound){
                                ...
                                searchText.setString("No Connection Was Found.");
                    ...
                    if(SearchButton2.isMouseOver(window)){
                        if(textbox1.numCharTyped!=0 && textbox2.numCharTyped!=0){
                            ...
                            if(OnOffButton2 && currentName3==textbox1.nameTyped && currentName4==textbox2.nameTyped){
                                ...
                                happy.setPosition({412.5,600});
                                happy.setTexture(TextureManager::GetTexture("mad"));
                                searchText.setString("You Have Already Searched These Names With DFS!");
                            ...
                            start = std::chrono::system_clock::now();
                            connectionFound2 = randGraph.dfsBool(got->second,got2->second);
                            end = std::chrono::system_clock::now();
                            std::chrono::duration<double> elapsed_seconds = end - start;
                            names = randGraph.DFSSearch(got->second,got2->second,connectionFound2);
                            double Etime=elapsed_seconds.count();
                            for(auto & name : names){
                                if(name.empty()){
                                    name="????";
                            ...
                            if(connectionFound2){
                                ...
                                searchText2.setPosition({175,575});
                                searchText2.setFillColor(Color::Green);
                                searchText2.setString("to compute the connection using DFS");
                            }
                            else{
                                searchText.setPosition({250,525});
                                searchText.setString("No Connection Was Found.");
                    ...
                    if(printConnection.isMouseOver(window)){
                        ...
                        if((textbox1.numCharTyped!=0 || textbox2.numCharTyped!=0) && print!=randGraph.stringmap.end() && print2!=randGraph.stringmap.end()) {
                            ...
                            randGraph.DFSPathfinder(print->second,print2->second,visited,path);
                            randGraph.printCompletePath(path);
                        }
                        else{
                            searchText.setPosition({360,525});
                            searchText.setString("Invalid Inputs");
                        }
                    }
            }
        }
